...
function PolicyWizard({ onClose, onCreated, onError }: { onClose: () => void; onCreated: (policy: Policy) => void; onError: (message: string) => void }) {
  ...
  const submit = () => {
    ...
    void create.mutateAsync({ policy: { customerId, policyType, premium: 0, status: 'NEW', startDate: null, endDate: null, underwritingDecision: null }, age, termMonths: term }).then((created) => onCreated(created)).catch((error: unknown) => onError(getErrorMessage(error, 'Unable to create policy.')));
  };
  return (
    <div className="feature-form">
      {create.error && <div className="form-error" role="alert">{getErrorMessage(create.error)}</div>}
      ...
      {step === 1 && (customers ? <div className="feature-form__field"><label htmlFor="policy-customer">Customer</label><select id="policy-customer" value={customerId} onChange={(e) => setCustomerId(Number(e.target.value))}><option value={0}>Select customer</option>{customers.map((item) => <option key={item.id} value={item.id}>{item.name} (risk {item.riskScore ?? '—'})</option>)}</select></div> : <SkeletonRows rows={2} />)}
...
function PolicyDetail({ policy }: { policy: Policy }) {
  ...
  return <div className="detail-layout"><div><Card title="Policy details"><dl className="detail-list"><div className="detail-list__row"><dt>Policy ID</dt><dd>#{policy.id}</dd></div><div className="detail-list__row"><dt>Type</dt><dd>{policy.policyType}</dd></div><div className="detail-list__row"><dt>Status</dt><dd><StatusBadge status={policy.status} /></dd></div><div className="detail-list__row"><dt>Premium</dt><dd>{formatCurrency(policy.premium)}</dd></div><div className="detail-list__row"><dt>Coverage period</dt><dd>{formatDate(policy.startDate)} – {formatDate(policy.endDate)}</dd></div><div className="detail-list__row"><dt>Underwriting</dt><dd>{policy.underwritingDecision ?? 'Pending'}</dd></div></dl></Card><Card title="Linked claims">{claims.isLoading ? <SkeletonRows /> : claims.isError ? <EmptyState icon="⚠️" title="Claims unavailable" description={getErrorMessage(claims.error)} /> : claims.data?.length ? <DataTable rows={claims.data} rowKey={(row) => String(row.id)} columns={[{ key: 'id', header: 'Claim', render: (value) => `#${value}` }, { key: 'claimAmount', header: 'Amount', render: (value) => formatCurrency(value as number) }, { key: 'status', header: 'Status', render: (value) => <StatusBadge status={String(value)} /> }]} /> : <EmptyState title="No claims filed." />}</Card></div><Card title="Billing history">{billing.isLoading ? <SkeletonRows /> : billing.isError ? <EmptyState icon="⚠️" title="Billing unavailable" description={getErrorMessage(billing.error)} /> : billing.data?.length ? <DataTable rows={billing.data} rowKey={(row) => String(row.id)} columns={[{ key: 'dueDate', header: 'Due', render: (value) => formatDate(value as string) }, { key: 'amount', header: 'Amount', render: (value) => formatCurrency(value as number) }, { key: 'paymentStatus', header: 'Status', render: (value) => <StatusBadge status={String(value)} /> }]} /> : <EmptyState title="No billing records." />}</Card></div>;
}
...
export function PolicyDetailPage() {
  ...
  if (policy.isError || !policy.data) return <EmptyState title="Policy not found" description={getErrorMessage(policy.error, 'The requested policy could not be loaded.')} />;
...
export function Policies() {
  ...
  return <div className="feature-page"><header className="feature-header"><div><h1 className="feature-title">Policies</h1><p className="feature-subtitle">Manage coverage worklists and review linked claims and billing.</p></div><button className="button button--primary" onClick={() => setWizardOpen(true)}>New policy</button></header><Card title="Policy worklist"><div className="toolbar"><div className="toolbar__field"><label htmlFor="policy-type-filter">Type</label><select id="policy-type-filter" value={type} onChange={(e) => setType(e.target.value)}><option value="ALL">All types</option>{POLICY_TYPES.map((item) => <option key={item}>{item}</option>)}</select></div><div className="toolbar__field"><label htmlFor="policy-status-filter">Status</label><select id="policy-status-filter" value={status} onChange={(e) => setStatus(e.target.value)}><option value="ALL">All statuses</option>{['ACTIVE', 'NEW', 'PENDING_REVIEW', 'ESCALATED', 'CANCELLED', 'EXPIRED'].map((item) => <option key={item}>{item}</option>)}</select></div><span className="feature-muted">{filtered.length} policies</span></div>{isLoading ? <SkeletonRows rows={6} /> : isError ? <EmptyState title="Policies unavailable" description={getErrorMessage(error, 'Check the API connection and try again.')} /> : filtered.length ? <DataTable rows={filtered} rowKey={(row) => String(row.id)} columns={columns} caption="Policy worklist" /> : <EmptyState icon="📋" title="No policies match these filters" />}</Card><Modal isOpen={wizardOpen} onClose={() => setWizardOpen(false)} title="New policy wizard" size="md"><PolicyWizard onClose={() => setWizardOpen(false)} onError={(message) => push(message, 'danger')} onCreated={(created) => { setWizardOpen(false); push(`Policy #${created.id} created.`, 'success'); navigate(`/policies/${created.id}`); }} /></Modal></div>;
}
